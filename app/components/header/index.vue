  ...
  <header class="sticky top-0 gap-4 bg-white z-50 px-4 md:px-6 bg-opacity-90 w-full">
    <section class="container flex justify-between h-16 items-center mx-auto">
      <div class="flex flex-row items-center gap-2">
        ...
        <NuxtLink to="/" class="text-xl text-uppercase cursor-pointer">
          <AppLogo />
        </NuxtLink>
      </div>
      <nav
        class="hidden justify-center w-full space-x-1 text-2xl font-medium lg:flex flex-row md:items-center md:space-x-2 md:text-lg lg:gap-6">
        <NuxtLink v-for="{ path, label } in navs" :key="path" :to="path" active-class="text-primary font-bold"
